...
namespace AIAudioTalesServer.Core.Services
{
    public class LibraryService : ILibraryService
    {
        ...
        {
            var domainBook = await _catalogService.GetBookAsync(bookId);
            if (domainBook == null) return false;

            return await _libraryRepository.AddToLibraryAsync(user, domainBook);
        }
        ...
        {
            await _libraryRepository.PurchaseBooks(userId, basketItems, purchaseType, language, sessionId);
        }
